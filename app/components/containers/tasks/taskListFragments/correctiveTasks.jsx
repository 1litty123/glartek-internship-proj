...
import { getActiveTasks, updateTask, endTask, getTask, getEventProfile, getProfiles } from '../taskActions'
import { endAlarm } from '../../alarms/alarmActions'
import { countFiles } from '../../files/fileActions'
import io from '../../../../utils/io'

class CorrectiveTasks extends React.Component {
    ...
    onSearchClick = async (filterFields) => {
        ...
        resp.rows.map(r => {
            if (r.taskCatalog.type === "corrective") {
                row.push(r)
                count++
            }
        })
    ...
    fetchActiveTasksData = (state, instance) => {
        this.setState({
            ...
            this.requestActiveTasksData()
                .then(resp => {
                    var row = []
                    resp.rows.map(r => { if (r.taskCatalog.type === "corrective") { row.push(r) } })
                    row.map(d => {
                        return axios.get(Consts.API_URL + "/" + d.taskCatalog.elementType + "s/" + d.taskCatalog.elementId + "?" + this.access_token)
                            .then(elementData => {
                                axios.get(Consts.API_URL + "/TaskInstances/" + d.id + "/events/" + d._events[0].id
                                    + "/by?" + "access_token=" + JSON.parse(localStorage.getItem('_user')).id).then(prof => {
...
const mapDispatchToProps = dispatch => bindActionCreators({ getProfiles, getTask, getActiveTasks, updateTask, endTask, endAlarm, getEventProfile, countFiles }, dispatch)

export default compose(translate("tasks"), connect(mapStateToProps, mapDispatchToProps))(CorrectiveTasks)
