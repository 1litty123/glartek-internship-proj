...
Object.freeze(tabsSub)

class PreventiveTasks extends React.Component {
    ...
    constructor(props) {
        ...
        this.activeSub = tabsSub.Periodic
        this.state = {
            dataWithElementName: [],
            filterFields: [],
            countPeriodic: 0,
    ...
    componentDidMount() {
        ...
        this.props.getProfileTeams()
            .then(teams => {
                this.managerTeams = teams.data
                this.requestCountChecklist()
            })
        this.setState({
            filterFields: []
        })
    ...
    onSearchClick = async (filterFields) => {
        ...
        resp.rows.map(r => {
            if (r.taskCatalog.type === "preventive") {
                row.push(r)
                count++
            }
        })
    ...
    onTabClick = (tab) => {
        this.activeSub = tab
        this.setState({
            type: Object.keys(tabsSub).find(k => tabsSub[k] === this.activeSub)
        })
    }
    ...
    requestCountChecklist = async () => {
        await this.props.getChecklists(this.managerTeams, this.state.tablestate)
    }
                    ...
                    row.map(d => {
                        return axios.get(Consts.API_URL + "/" + d.taskCatalog.elementType + "s/" + d.taskCatalog.elementId + "?" + this.access_token)
                            .then(elementData => {
                                ...
                                merged.push(Object.assign({ elementName: elementData.data.name }, d))
                                this.setState({
                        ...
                        <div className="ibox float-e-margins">
                            <div className="tabs-container">
                                <ul className="nav nav-tabs">
                                    <li className={this.activeSub === tabsSub.Periodic ? "active" : ""}>
                                        <a data-toggle="tab" onClick={() => this.onTabClick(tabsSub.Periodic)}>
                                            {t("historic:labels.periodic") + " (" + this.state.countPeriodic + ")"}
                                        </a>
                                    </li>
                                    <li className={this.activeSub === tabsSub.Checklist ? "active" : ""}>
                                        <a data-toggle="tab" onClick={() => this.onTabClick(tabsSub.Checklist)}>
                                            {t("historic:labels.checklists") + " (" + (this.props.checklists.checklists.rows.length ? this.props.checklists.checklists.rows.length : "0") + ")"}
                                        </a>
                                    </li>
                                </ul>
                            </div>
                            {this.activeSub === tabsSub.Periodic ?
                                <div className="ibox-content" style={{ borderTop: '1px solid white' }}>
                                    <div className="tab-content">
                                        <SearchManager count={this.state.countPeriodic} data={active.currentData}
                                            callback={this.onSearchClick} whatSearch={["assignedToType", "state", "createdAt"]} />
                                ...
                                : <ChecklistList teams={this.managerTeams} />}
                        </div>
                    </div>
                </div>
            </div>
...
const mapDispatchToProps = dispatch => bindActionCreators({
    getSeverities, getChecklists, getProfileTeams, getTask, getActiveTasks,
    updateTask, endTask, endAlarm, getEventProfile, countFiles
}, dispatch)

export default compose(translate("tasks"), connect(mapStateToProps, mapDispatchToProps))(PreventiveTasks)
